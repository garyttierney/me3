...
impl KnownDirs {
    ...
    /// Discover the cache directory. This location is used to store log files.
    pub fn data_dir(&self) -> Option<Box<Path>> {
        self.project_dirs
            .as_ref()
            .map(|dirs| dirs.data_local_dir().to_path_buf().into_boxed_path())
            .or(self.cwd.clone())
    }
    ...
    pub fn windows_bin_dirs(&self) -> impl Iterator<Item = PathBuf> {
        ...
        let windows_bin_dirs = [
            ...
            self.project_dirs
                .as_ref()
                .map(|dirs| dirs.data_local_dir().join("windows-bin")), /* Installed via
                                                                         * installer.sh */
    ...
    pub fn config_dirs(&self) -> impl Iterator<Item = Box<Path>> {
        let config_dirs = [
            ...
            self.project_dirs
                .as_ref()
                .map(|proj| proj.config_local_dir().to_path_buf().into_boxed_path()),
        ];
