...
This article focuses on the reverse engineering and declawing of the Arxan anti-debug and tamper protection software as it is used in FROMSOFTWARE games. It assumes basic x86 assembly, low-level reverse engineering and Rust knowledge.
...
3. The actual logic of the stub is invoked using a `CALL` (which may be obfuscated). If it's an *obfuscated stub*, successful completion of this routine will correct return gadgets over the 3 bogus return addresses from step 1.
...
- It's quite slow: Some of the stubs would take a few dozen milliseconds to fully execute, which isn't ideal when you have to go through 3000 of them at runtime when the game boots (of course static analysis would still be within reach).
...
#### Anti-Debug Return Address Cloberring
For the Arxan versions present in games past Dark Souls Remastered, anti-debug stubs have a new mechanism for crashing the process: overwriting the return address of their main logic function with one that points to invalid memory. This is a problem as this function has a common "end" block, so if the "bad" bad is taken first (which is the case), the "good" path won't return either as it will stop on previously visited instructions.
...
While this was enough to return from anti-debug stubs in games up to Elden Ring, it proved insufficient for Armored Core 6 and Elden Ring: Nightreign. The Arxan developers likely changed the logic to a point where the return address is cloberred earlier, many conditional branches before the return. 
...
- always the second and third one `RET`-like instruction sequences, so only the first one is relevant :facepalm:

This makes analysis significantly easier. We just need to grab the first above-initial-stack write, so it doesn't matter if some obscure deeply nested paths of the stub's CFG can't aren't visited:
